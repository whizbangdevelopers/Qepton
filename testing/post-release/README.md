...
Verification issues are **auto-created** when a release is published. See `.github/workflows/release-verify-create.yml`.

### How It Works

1. Release is published → 7 issues auto-created (1 parent + 6 sub-issues)
2. Sub-issues assigned per test environment (Windows, macOS, Ubuntu, Fedora VM, Arch VM, PWA)
3. Tester works through checklist, closes issue when complete
4. Closing a sub-issue automatically updates the README verification table
5. When all sub-issues closed, parent auto-closes

### Finding Verification Issues

```bash
# List all verification issues for a version
gh issue list --label release-verify --search "v1.0.13"

# View the parent tracker
gh issue list --label release-verify-parent --search "v1.0.13"
```
...
3. File a **new** bug issue in Qepton-Dev with `bug` label
4. Link the bug in the verification sub-issue before closing
...
If automated issues weren't created, trigger manually:

```bash
gh workflow run release-verify-create.yml -f release_tag=v1.0.13
```

Or use the issue template: **New Issue → Release Verification (Manual)**
